...
def find_device(device_data, user_serial):
    ...
    matched_device = next((device for device, serial in all_devices.items() if serial == user_serial), None)
...
def main():
    ...
    if st.button("Find Devices") and device_serial and api_key:
        ...
        matched_device, all_device_serials = find_device(device_data, device_serial)
        ...
        st.write(f"Matching device for provided serial {device_serial} is {matched_device}" if matched_device else "No matching devices found for the provided serial.")
    ...
    selected_device = st.selectbox("Select a device:", st.session_state.devices)
    selected_device_first_char_capitalized = selected_device[0].upper() if selected_device else None
    ...
    if st.button("Fetch Consumption Data") and selected_device and device_serial and api_key:
        selected_device_serial = st.session_state.device_to_serial_map.get(selected_device)
        if selected_device_serial:
            with st.spinner('Fetching consumption data...'):
                consumption_data = get_consumption_data(selected_device_first_char_capitalized, selected_device_serial, device_serial, api_key, start_date, end_date)
            ...
            st.line_chart(df.set_index('time')[['v1', 'frq']].rename(columns={'v1': 'Voltage', 'frq': 'Frequency Hz'}))

            st.subheader("Tank Temperatures:")
            st.line_chart(df.set_index('time')[['pt1', 'pt2']].rename(columns={'pt1': 'Tank 1 Temperature C', 'pt2': 'Tank 2 Temperature C'}))
            ...
            col1.metric("Total Imported Energy", "{:.2f} kWh".format(df['imp_kWh_metric'].iloc[0]))  # using iloc[0] to just get the single value from the dataframe
            col1.metric("Total Exported Energy", "{:.2f} kWh".format(df['exp_kWh_metric'].iloc[0]))
            col1.metric("Total Generated Energy", "{:.2f} kWh".format(df['gep_kWh_metric'].iloc[0]))
            col1.metric("Total Consumed Energy", "{:.2f} kWh".format(df['gen_kWh_metric'].iloc[0]))

            col2.metric("Average Voltage", "{:.2f} V".format(df['v1'].mean()))
            col2.metric("Average Frequency", "{:.2f} Hz".format(df['frq'].mean()))
            col2.metric("Average Tank Temperature Sensor 1", "{:.2f} °C".format(df['pt1'].mean()))
            col2.metric("Average Tank Temperature Sensor 2", "{:.2f} °C".format(df['pt2'].mean()))

        else:
            st.warning("Could not find a serial number for the selected device.")
    else:
        st.warning("Please select a device, and ensure Device Serial and API Key are provided.")
